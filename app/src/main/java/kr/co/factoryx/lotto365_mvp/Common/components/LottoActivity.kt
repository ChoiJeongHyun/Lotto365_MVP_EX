...
abstract class LottoActivity : FragmentActivity(), View.OnClickListener {
    ...
    override fun setContentView(layoutResID: Int) {
        contentV = LayoutInflater.from(this).inflate(layoutResID, null)
    ...
    private fun setEnterTransition() {
        ...
        if (transition == Transition.SLIDE_SIDE) overridePendingTransition(R.anim.right_in, R.anim.left_out)
        if (transition == Transition.SLIDE_UP) overridePendingTransition(R.anim.bottom_in, R.anim.no)
    }

    private fun setExitTransition() {
        transition = Transition.values()[getInt("ACTIVITY_TRANSITION")]
        if (transition == Transition.NO_ANIMATION) overridePendingTransition(0, 0)
        if (transition == Transition.SLIDE_SIDE) overridePendingTransition(R.anim.left_in, R.anim.right_out)
        if (transition == Transition.SLIDE_UP) overridePendingTransition(R.anim.no, R.anim.bottom_out)
    }
    ...
    fun setTransition(transition: Transition): LottoActivity {
        this.transition = transition
        return this
    }

    private fun putTransitionToBundle(intent: Intent, transition: Transition) = intent.putExtra("ACTIVITY_TRANSITION", transition.ordinal)
